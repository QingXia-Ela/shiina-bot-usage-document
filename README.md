...
目前已收录计算机、土木、机械专业的圣经，欢迎补充

用法：`[关键词]圣经`，例如：计算机圣经、土木圣经、机械圣经

### b站用户动态监听与直播提醒

用法：

- `#bili listen [uid]` 开启该用户动态与直播监听
- `#bili unlisten [uid]` 关闭该用户动态与直播监听
- `#bili help` 查看帮助

举例：`#bili listen 1605958838` 可以开启直播间监听

用户 uid 获取方法

- 在浏览器打开你想要监听主播的直播间
- 然后看地址栏，类似这种：https://space.bilibili.com/1605958838，后面如果有带个 `?` 号的可以直接忽略
- 地址栏那一串数字就是用户 uid，假如我想要订阅以上直播间开播通知，我就可以这样子发送指令：`#bili listen 1605958838`
